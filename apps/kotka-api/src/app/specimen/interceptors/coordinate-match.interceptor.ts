...
export class CoordinateMatchInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<Document> {
    ...
    if ((
      ...
      && !coordinateSystem && !wgs84Latitude
      && !wgs84Longitude ) || (
      originalLatitude && originalLongitude
      && coordinateSystem && !wgs84Latitude
      && !wgs84Longitude
      ...
      throw new HttpException(getError('/gatherings/0', 'Missing expected coordinate data'), HttpStatus.UNPROCESSABLE_ENTITY);
    }
    ...
    if (!convertedCoordinates) {
      throw new HttpException(getError('/gatherings/0', 'Unable to convert original coordinate data'), HttpStatus.UNPROCESSABLE_ENTITY);
    }
    ...
    if (Math.abs(convertedCoordinates[0] - Number(wgs84Latitude)) > COORDINATE_DELTA || Math.abs(convertedCoordinates[1] - Number(wgs84Longitude)) > COORDINATE_DELTA) {
      throw new HttpException(getError('/gatherings/0', 'Provided WGS84 coordinates do not match original coordinates'), HttpStatus.UNPROCESSABLE_ENTITY);
    }
