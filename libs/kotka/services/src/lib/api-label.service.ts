...
export class ApiLabelService {
  ...
  getLabels(keys: string[], type: ApiLabelType): Observable<Record<string, string>> {
    ...
    const { setCachedLabelsToResult$, missingKeys } = this.setCachedLabelsToResult(result, validKeys);
  ...
  private setCachedLabelsToResult(result: Record<string, string>, keys: string[]): {
    ...
    return {
      setCachedLabelsToResult$: (observables.length > 0 ? forkJoin(observables) : of([])),
      missingKeys
    };
  }

  private fetchAndCacheLabels(keys: string[], type: ApiLabelType): Observable<Record<string, string>> {
  ...
  private fetchLabels(keys: string[], type: ApiLabelType): Observable<{ key: string; value: string; }[]> {
