...
export class AbschService {
  ...
  getCountryLinks(country?: string): Observable<LinkData[]> {
    ...
    if (!this.countryLinksCache[country]) {
      const url = this.countryLinksUrl.replace('%country%', country.toLocaleLowerCase());
      this.countryLinksCache[country] = this.httpClient.get<CountryLinkResult>(url).pipe(
        map(result => {
