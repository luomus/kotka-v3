...
export class TransactionCountRendererComponent extends CellRendererComponent<RendererParams> {
  ...
  override paramsChange() {
    ...
    this.result = TransactionCountRendererComponent.getTransactionCount(
      transaction,
      this.params.type,
    );
  }
  ...
  static getTransactionCount(
    ...
    const awayCount =
      (transaction.awayIDs || []).length + (transaction.awayCount || 0);
    const returnedCount =
      (transaction.returnedIDs || []).length + (transaction.returnedCount || 0);
    const missingCount =
      (transaction.missingIDs || []).length + (transaction.missingCount || 0);
